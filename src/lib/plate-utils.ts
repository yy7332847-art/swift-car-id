...
function normalizeArabic(input: string): string {
  return input.trim().toLowerCase().replace(/[\u064B-\u0652\u0670\u0640]/g, "").split("").map((c) => AR_DIGITS[c] ?? LETTER_MAP[c] ?? c).join("");
}
...
export function displayPlateLetter(ch: string): string {
  if (ch === "ا") return "أ";
  if (ch === "ه") return "هـ";
  return ch;
}
...
export function formatPlateParts(letters: string, digits: string): string {
  return `${letters.split("").map(displayPlateLetter).join(" ")} ${digits}`.trim();
}

const DIGIT_WORDS: Record<string, string> = {
  "0": "0", "zero": "0", "صفر": "0", "صفره": "0", "زيرو": "0", "سفر": "0",
  "1": "1", "واحد": "1", "احد": "1", "احدى": "1", "احدا": "1", "ون": "1",
  "2": "2", "اثنين": "2", "اتنين": "2", "تنين": "2", "ثنين": "2", "اثنان": "2", "اثنا": "2", "اثني": "2",
  "3": "3", "ثلاثه": "3", "ثلاثة": "3", "ثلاث": "3", "تلاته": "3", "تلاتة": "3", "تلات": "3",
  "4": "4", "اربعه": "4", "اربعة": "4", "اربع": "4", "ربعه": "4", "ربعة": "4",
  "5": "5", "خمسه": "5", "خمسة": "5", "خمس": "5",
  "6": "6", "سته": "6", "ستة": "6", "ست": "6",
  "7": "7", "سبعه": "7", "سبعة": "7", "سبع": "7",
  "8": "8", "ثمانيه": "8", "ثمانية": "8", "ثمان": "8", "تمانيه": "8", "تمانية": "8", "تمنيه": "8", "تمنية": "8", "تمن": "8",
  "9": "9", "تسعه": "9", "تسعة": "9", "تسع": "9",
};

const UNITS: Record<string, number> = Object.fromEntries(Object.entries(DIGIT_WORDS).filter(([, v]) => /^\d$/.test(v)).map(([k, v]) => [k, Number(v)]));
...
// Official Saudi plate letters only. Internally "أ" is normalized to "ا" for matching.
const PLATE_LETTERS = new Set("ابحدرسصطعقكلمنهوي".split(""));
const LETTER_NAMES: Record<string, string> = {
  "الف": "ا", "الالف": "ا", "ا": "ا", "أ": "ا",
  "باء": "ب", "با": "ب", "بي": "ب", "ب": "ب",
  "حاء": "ح", "حا": "ح", "حي": "ح", "ح": "ح",
  "دال": "د", "دا": "د", "دي": "د", "د": "د",
  "راء": "ر", "را": "ر", "ري": "ر", "ر": "ر",
  ...
  "ميم": "م", "مي": "م", "م": "م",
  "نون": "ن", "نو": "ن", "ني": "ن", "ن": "ن",
  "هاء": "ه", "ها": "ه", "هي": "ه", "هيه": "ه", "ه": "ه",
  "واو": "و", "وو": "و", "و": "و",
  "ياء": "ي", "يا": "ي", "يي": "ي", "ي": "ي",
};
const COMMON_NON_PLATE_WORDS = new Set(["انا", "انت", "انتي", "انه", "اني", "هذا", "هذه", "اللي", "على", "علي", "السلام", "عليكم", "ورحمه", "ورحمة", "الله", "وبركاته", "تمام", "يعني", "كده", "اه", "ايوه", "اشوف", "شوف", "دي", "ده"]);
const NUMBER_PREFIX_NOISE = new Set(["رقم", "ارقام", "نمره", "نمرة", "لوحه", "لوحة"]);
...
function parseLetterToken(raw: string): string | null {
  const w = normalizeArabic(raw);
  if (!w || COMMON_NON_PLATE_WORDS.has(w)) return null;
  const clean = w.startsWith("و") && w.length > 2 ? w.slice(1) : w;
  const named = LETTER_NAMES[clean] ?? LETTER_NAMES[w];
  if (named && PLATE_LETTERS.has(named)) return named;
  if (clean.length === 1) {
    const ch = LETTER_MAP[clean] ?? clean;
    if (PLATE_LETTERS.has(ch)) return ch;
  }
  return null;
}
...
function parseArabicNumberRun(words: string[], startIdx: number): { value: string; consumed: number; suspectPart?: string; correctionNote?: string } {
  type Cand = { value: string; consumed: number; suspectPart?: string; correctionNote?: string; priority: number };
  const candidates: Cand[] = [];

  // Strategy A: plate-safe digit spelling: "واحد تسعة تسعة صفر" / "1 9 9 0".
  let seq = "", seqConsumed = 0;
  for (let i = startIdx; i < words.length && seq.length < 4; i++) {
    const raw = stripWa(words[i] ?? "");
    const direct = directDigits(raw);
    if (direct) {
      seq += direct;
      seqConsumed++;
      continue;
    }
    const digit = DIGIT_WORDS[raw];
    if (digit === undefined) break;
    seq += digit;
    seqConsumed++;
  }
  if (seq.length >= 1) candidates.push({ value: seq.slice(0, 4), consumed: seqConsumed, priority: 4 });

  // Strategy B: two-digit spoken groups: "عشرين تلاتين" => 2030.
  const groups: string[] = [];
  let groupConsumed = 0;
  for (let i = startIdx; i < words.length && groups.length < 2;) {
    const g = parseTwoDigitGroup(words, i);
    if (!g) break;
    groups.push(g.value.toString().padStart(2, "0"));
    i += g.consumed; groupConsumed += g.consumed;
  }
  if (groups.length >= 1) candidates.push({ value: groups.join("").slice(0, 4), consumed: groupConsumed, priority: groups.length >= 2 ? 3 : 1 });

  // Strategy C: natural number after the letter stage only: "الف تسعمية وتسعين" => 1990.
  const nat = parseNaturalNumber(words, startIdx);
  if (nat.value) candidates.push({ value: nat.value.slice(-4), consumed: nat.consumed, priority: THOUSANDS.has(stripWa(words[startIdx] ?? "")) ? 2 : 1 });

  // Special: two consecutive hundreds words ("ميتين ميتين" = 2200).
  ...
  candidates.sort((a, b) => b.priority - a.priority || Math.min(4, b.value.length) - Math.min(4, a.value.length) || b.consumed - a.consumed);
  const best = candidates[0];
  return { value: best.value.slice(0, 4), consumed: best.consumed, suspectPart: best.suspectPart, correctionNote: best.correctionNote };
}
...
function pushFound(found: DetectedPlate[], plate: Omit<DetectedPlate, "raw" | "normalized" | "complete" | "confidence"> & { confidence?: number }) {
  ...
  const suspectPart = plate.suspectPart ?? (!complete ? `ينقص ${letters.length < 3 ? `${3 - letters.length} حرف` : ""}${letters.length < 3 && digits.length < 4 ? " و " : ""}${digits.length < 4 ? `${4 - digits.length} رقم` : ""}` : undefined);
  found.push({ raw: formatPlateParts(letters, digits), normalized, letters, digits, complete, confidence: plate.confidence ?? (complete ? 0.92 : 0.56), suspectPart, correctionNote: plate.correctionNote });
}
...
export function extractPlates(text: string): DetectedPlate[] {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    while (j < words.length && letters.length < 3) {
      const ch = parseLetterToken(words[j]);
      if (ch) { letters += ch; j++; continue; }
      break;
    }
    if (letters.length >= 1) {
  ...
  for (const w of words) {
    const m = normalizePlate(w).match(/^([\u0621-\u064A]{2,3})(\d{1,6})$/);
